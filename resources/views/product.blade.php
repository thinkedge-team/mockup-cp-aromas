@php
    ...
    $brands = ProductBrand::active()
        ->whereHas('brandCategories', function ($q) {
            $q->where('is_active', true)->whereHas('category', fn($c) => $c->where('is_active', true));
        })
        ->with([
            'brandCategories' => function ($q) {
    ...
    $whatsappNumber =
        $footer && isset($footer->contact_info['whatsapp'])
...
@section('meta_description',
    'Produk AROMAS – Minyak goreng sawit berkualitas dalam kemasan Botol, Jeriken, dan BIB
    untuk kebutuhan rumah tangga hingga industri.')
    ...
    @push('styles')
        <style>
            ...
            .dc1 {
                ...
                top: -220px;
                right: -130px;
                animation: rotSlow 38s linear infinite;
            }
            ...
            .dc3 {
                ...
                border-color: rgba(244, 196, 48, .12);
                animation: pRing 4.5s ease-in-out infinite;
            }
            ...
            .ftab {
                ...
                font-size: .88rem;
                font-weight: 600;
                border: 2px solid rgba(34, 139, 34, .2);
            ...
            .category-tabs-wrapper {
                background: linear-gradient(135deg, #f8faf8, #fffdf8);
                border-radius: 16px;
                padding: 20px 24px;
            ...
            .cat-tab.active {
                ...
                color: #fff;
                border-color: transparent;
                box-shadow: 0 3px 12px rgba(34, 139, 34, .25);
            }
            ...
            .prod-card:hover {
                ...
                box-shadow: 0 18px 52px rgba(34, 139, 34, .16);
            ...
            .card-img-wrap {
                ...
                background: linear-gradient(135deg, #f5faf5, #fafff5);
                height: 230px;
            }
            ...
            /* SIZES GRID (Botol) */
            .sizes-showcase {
                background: linear-gradient(135deg, #f5f9f5, #fffdf7);
                border-radius: 20px;
                padding: 32px;
                border: 1px solid rgba(34, 139, 34, .1);
                margin-bottom: 20px;
            ...
            .size-item:hover {
                border-color: var(--green);
                box-shadow: 0 4px 16px rgba(34, 139, 34, .12);
            ...
            .modal-img-band {
                background: linear-gradient(135deg, #f5faf5, #fffdf7);
                border-radius: 16px;
                padding: 28px;
                text-align: center;
                margin-bottom: 26px;
            ...
            .kelebihan-section {
                ...
                background: linear-gradient(135deg, #1a2e1a, #0d3320 55%, #15412a);
                position: relative;
                overflow: hidden;
            }

            .kelebihan-section::before {
                content: '';
                position: absolute;
                inset: 0;
                background: radial-gradient(ellipse at 30% 50%, rgba(212, 160, 23, .12) 0%, transparent 55%), radial-gradient(ellipse at 70% 80%, rgba(34, 139, 34, .18) 0%, transparent 45%);
                pointer-events: none;
            }
            ...
            /* Match .card-img-wrap height */
            .product-modal-img-placeholder {
                height: 250px;
            }

            /* Match modal-img-band img max-height */
        </style>
    @endpush
...
@section('content')
        ...
        <div class="container text-center">
            <div class="row justify-content-center">
                <div class="col-lg-8 col-md-10" data-aos="fade-up">
                    @if ($productHero)
                        <span class="hero-badge"><i class="bi {{ $productHero->badge_icon }}"></i>
                            {{ $productHero->badge_text }}</span>
                        <h1 class="hero-title">
                            {!! $productHero->title !!}
                    ...
                    <div class="hero-chips">
                        @foreach ($brands as $brand)
                            <span class="chip @if ($loop->first) active @endif"
    ...
    <section class="cat-section">
        <div class="container" id="productsContainer">
            @foreach ($brands as $brand)
                <div class="brand-block" data-brand="{{ $brand->slug }}"
                    ...
                    @forelse($brand->brandCategories as $brandCategory)
                                                ...
                                                <div class="card-body-inner">
                                                    <div class="card-sizes">
                                                        @foreach (array_slice($product->sizes ?? [], 0, 5) as $size)
                                                            <span
                                                                class="size-tag @if ($size['is_popular'] ?? false) highlight @endif">
                                                    ...
                                                    <ul class="card-features">
                                                        @foreach (array_slice($product->features ?? [], 0, 3) as $feature)
                                                            <li><i
                                                                    class="bi bi-check-circle-fill"></i>{{ $feature['text'] }}
                                                            </li>
                                                        @endforeach
                                                    </ul>
                                                    <div class="card-footer-row">
                                                        <span class="btn-detail"><i class="bi bi-eye"></i> Detail</span>
                                                        <a href="https://wa.me/{{ $whatsappNumber }}?text={{ urlencode($product->whatsapp_message ?? 'Halo, saya tertarik dengan ' . $product->name) }}"
                                                            target="_blank" class="btn-wa-card"
                        ...
                        @if (!$loop->last)
                            <hr style="border:0;border-top:1px solid var(--gray-200);margin:60px 0;" class="cat-divider"
                                data-brand="{{ $brand->slug }}">
                        @endif
                    @empty
                        <div class="text-center py-5">
    ...
    <section class="kelebihan-section">
        <div class="container">
            <div class="text-center mb-5" data-aos="fade-up">
                <h2 class="section-title" style="color:#fff;">Mengapa Memilih <span
                        class="italic text-gradient">AROMAS?</span></h2>
                    ...
                    <div class="d-flex gap-3 flex-wrap justify-content-lg-end">
                        @if ($productCta)
                            <a href="{{ $productCta->primary_button_url }}" target="_blank" class="btn-cta-w">
                                <i class="bi {{ $productCta->primary_button_icon ?? 'bi-whatsapp' }}"></i>
                                {{ $productCta->primary_button_text }}
                            </a>
                            <a href="{{ $productCta->secondary_button_url }}" class="btn-cta-ol">
                                <i class="bi {{ $productCta->secondary_button_icon ?? 'bi-envelope-fill' }}"></i>
                                {{ $productCta->secondary_button_text }}
                            </a>
                        @else
                            <a href="https://wa.me/{{ $whatsappNumber }}?text=Halo%20AROMAS,%20saya%20ingin%20tanya%20harga%20produk"
    ...
    @foreach ($brands as $brand)
        @foreach ($brand->brandCategories as $brandCategory)
            ...
            @foreach ($brand->products()->where('category_id', $category->id)->active()->get() as $product)
                <div class="prod-modal" id="modal-product-{{ $product->id }}" role="dialog" aria-modal="true"
                    ...
                    <div class="modal-box">
                        ...
                        <button class="modal-close" onclick="closeModal('product-{{ $product->id }}')"><i
                                class="bi bi-x-lg"></i></button>
                        <div class="modal-inner">
                            <div class="modal-tag">{{ $brand->name }} · {{ $category->label }} ·
                                {{ $product->badge_text }}</div>
                                        ...
                                        <div class="product-image-slider" data-product-id="{{ $product->id }}">
                                            ...
                                            @if (count($images) > 1)
                                                <button class="slider-prev"
                                                    onclick="slideImage({{ $product->id }}, -1)"><i
                                                ...
                                                <div class="slider-dots">
                                                    @foreach ($images as $index => $image)
                                                        @if (!empty($image['url']))
                                                            <span
                                                                class="slider-dot {{ $index === $bannerIndex ? 'active' : '' }}"
                            ...
                            <div class="modal-actions">
                                <a href="https://wa.me/{{ $whatsappNumber }}?text={{ urlencode($product->whatsapp_message ?? 'Halo, saya ingin pesan ' . $product->name) }}"
                                    target="_blank" class="btn-modal-wa"><i class="bi bi-whatsapp"></i> WhatsApp</a>
...
@push('scripts')
    ...
    <script>
        ...
        function updateCategoryTabs(brandSlug) {
            ...
            var brandData = brandsData.find(function(b) {
                return b.slug === brandSlug;
            });

            if (!brandData || !categoryTabsContainer) return;

            var categories = brandData.categories;
            var totalProducts = categories.reduce(function(sum, cat) {
                return sum + cat.productCount;
            }, 0);
